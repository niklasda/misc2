...
namespace EV.Windows.Forms
{
    ...
    public class ListViewTextSort: IComparer
    {
        ...
        protected Int32 m_column;
        protected Boolean m_ascending;
    }

    public class ListViewFileFolderSort: ListViewTextSort
    {
        /// <summary>
        /// Constructor
        /// </summary>
        /// <param name="sortColumn">Column to be sorted</param>
        /// <param name="ascending">true, if ascending order, false otherwise</param>
        public ListViewFileFolderSort(Int32 sortColumn, Boolean ascending):
        ...
        /// <param name="rhs">Second Object to compare</param>
        /// <returns>Less that zero if lhs is less than rhs. Greater than zero if lhs greater that rhs. Zero if they are equal</returns>
        public override Int32 Compare(Object lhs, Object rhs)
        {
            ListViewItem lhsLvi = lhs as ListViewItem;
            ListViewItem rhsLvi = rhs as ListViewItem;
            ...
            if(lhsLvi.Tag == null || rhsLvi.Tag == null)    
                return 0;

            ShellDll.ShellItem lhsItems = lhsLvi.Tag as ShellDll.ShellItem;
            ShellDll.ShellItem rhsItems = rhsLvi.Tag as ShellDll.ShellItem;
              
            if(lhsItems == null || rhsItems == null)    
                return 0;


            Int32 result = 0;
            if(lhsItems.IsDisk || rhsItems.IsDisk )
            {
                return 0;
            }
            else if(lhsItems.IsFolder != rhsItems.IsFolder )
            {
                result = rhsItems.IsFolder ? 1 : -1;
            }
            else
            {
                result = OnCompare(lhsItems.Text, rhsItems.Text);
    ...
    public class ListViewDateSort: ListViewTextSort
    {
        ...
        protected override Int32 OnCompare(String lhs, String rhs)
        {
            DateTime lRes;
            DateTime rRes;
            bool lSuc = DateTime.TryParse(lhs, out lRes);
            bool rSuc = DateTime.TryParse(rhs, out rRes);
