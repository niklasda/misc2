    ...
    public partial class Browser : UserControl
    {
        ...
        private string ConvertPath(string path)
        {
            ...
            string newPath = path.Trim();
             ...
             if (newPath.StartsWith(@"\",true, CultureInfo.InstalledUICulture))
        ...
        {
            return NavAddressBox.Text;

      /*      string fullPath = SelectedNode.FullPath;
            fullPath = fullPath.Replace(SpecialFolders.Desktop.ToString(), "");
        ...
        /// </summary>
        /// <param name="item">The ShellItem to select</param>
        /// <returns>The TreeNode of the directory which was selected, this will be null if the directory
        /// doesn't exist</returns>
        ...
        public bool DeleteFiles()
        {
            ...
            pidls = new IntPtr[FileView.SelectedItems.Count];
            for (int i = 0; i < pidls.Length; i++)
            {
                pidls[i] = ((ShellItem)FileView.SelectedItems[i].Tag).PIDLRel.Ptr;
            }
            parent = SelectedItem;
        ...
        public bool DeleteFolder()
        {
            ...
            pidls = new IntPtr[1];
            pidls[0] = SelectedItem.PIDLRel.Ptr;
            parent = (SelectedItem.ParentItem != null ? SelectedItem.ParentItem : SelectedItem);
        ...
        private void navDeleteButton_Click(object sender, EventArgs e)
        {
            if(lastClicked == LastView.File)
                DeleteFiles();
            else if (lastClicked == LastView.Folder)
                DeleteFolder();
        }
        ...
        private IBrowserContainer parentContainer;
        public void SetParentContainer(IBrowserContainer container)
        {
            parentContainer = container;
        }

        private enum LastView { File, Folder, None };
        private LastView lastClicked = LastView.None;

        private void fileView_Click(object sender, EventArgs e)
        {
            lastClicked = LastView.File;
            if(parentContainer != null)
                parentContainer.SetActiveBrowser(this);
  //         FormMain fm  = (FormMain)this.Parent.Parent.Parent;
        }

        private void folderView_Click(object sender, EventArgs e)
        {
            lastClicked = LastView.Folder;
            if(parentContainer != null)
                parentContainer.SetActiveBrowser(this);
        }
